      ...
      <form onSubmit={handleSearch} className="w-full">
        <div className="relative">
          <input
            ...
            className="w-full bg-background border border-color rounded-full py-3 px-5 text-foreground focus:outline-none focus:ring-2 focus:ring-primary shadow-sm"
            value={searchQuery}
          ...
          {showResults && (
            <div className="absolute mt-1 w-full bg-secondary border border-color rounded-md shadow-lg z-50 max-h-80 overflow-y-auto">
              {isSearching ? (
                ...
                <ul>
                  {searchResults.map((user) => (
                    <li 
                      key={user._id}
                      className="cursor-pointer hover:bg-background hover:border-primary border-transparent border p-3"
                    ...
                    >
                      <button 
                        ...
                        onClick={(e) => navigateToProfile(user.username, e)}
                      >
                        <div className="w-8 h-8 rounded-full bg-foreground flex items-center justify-center mr-3">
                          {user.image ? (
                            <img
                              src={user.image}
                              alt={user.name}
                ...
                <p className="p-3 text-center text-secondary">No se encontraron usuarios</p>
              )}
            </div>
          )}
        </div>
